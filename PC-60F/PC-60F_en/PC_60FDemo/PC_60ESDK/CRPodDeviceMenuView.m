...
/** PC-60E device menu item options*/
...
/** Block executed after clicking update */
...
/** The pickerView for selecting values */
@property (nonatomic, strong) UIPickerView *pickerView;
/** The value range of pickerView */
...
- (void)initSubviewsWithPort:(NSString *)deviceName
{
    ...
    tipsL.string = NSLocalizedString(@"Device menu item",@"Device menu item");
    [infoView.layer addSublayer:tipsL];
    // Currently connected device：
   CATextLayer *descriptText1 = [self createTextLayerWithFrame:CGRectMake(margin, CGRectGetMaxY(tipsL.frame) + margin, 0.4 * size.width, lineHeight) Text: NSLocalizedString(@"Currently connected device:", @"Currently connected device:") FontSize:15];
    //设备名称
...
//    _portNameLayer.alignmentMode = kCAAlignmentLeft;
    ...
    [updateBtn setTitle:NSLocalizedString(@"Update", @"Update") forState:UIControlStateNormal];
...
{
    _spot = spot;
    _spotLayer.string = [NSString stringWithFormat:@"%@", spot == 1 ? @"Spot check" : @"Continuous"];
}

- (void)setBeepOn:(int)beepOn
{
    _beepOn = beepOn;
    _beepOnLayer.string = [NSString stringWithFormat:@"%@", beepOn == 1 ? @"On" : @"Off"];
}

- (void)setRotateOn:(int)rotateOn
{
    _rotateOn = rotateOn;
    _rotateOnLayer.string = [NSString stringWithFormat:@"%@", rotateOn == 1 ? @"On" : @"Off"];
}
...
-(NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component
{
    ...
    if (self.option == CRPC_60EDeviceMenuOptionSpot)
        title = title.intValue == 1 ? @"Spot check" : @"Continuous";
    else if (self.option == CRPC_60EDeviceMenuOptionBeepOn)
        title = title.intValue == 1 ? @"On" : @"Off";
    else if (self.option == CRPC_60EDeviceMenuOptionRotateOn)
        title = title.intValue == 1 ? @"On" : @"Off";
    return title;
}
// The selected row is displayed on the label
...
// Set the initially selected row as the current value
